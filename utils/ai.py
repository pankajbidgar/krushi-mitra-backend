...
def get_ai_response(prompt: str) -> str:
    ...
    try:
        response = client.models.generate_content(
            # model="gemini-2.0-flash-exp", 
            model="gemini-2.0-flash-lite-001", # हे सध्याचे फ्री मॉडेल
            contents=prompt,
            config=types.GenerateContentConfig(
                system_instruction="तू 'Krushi Mitra' आहेस, एक शेती सहाय्यक. फक्त शेतीविषयक प्रश्नांना मराठीत उत्तर दे."
            )
        )
        return response.text
    except Exception as e:
        print(f"AI API error: {e} (fallback to rule-based)")
